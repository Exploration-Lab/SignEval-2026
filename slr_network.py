...
# import utils
# from modules.temporal_layers import BiLSTMLayer, TemporalConv,  TemporalTransformerEncoder
...
#     def __init__(self, visual_args, gloss_dict, conv_type, loss_weights, norm_scale=32,temporal_encoder = "bilstm") -> None:
#         super().__init__()
...
#         print(f"\n===== Temporal Encoder Selected: {self.temporal_encoder} =====\n")
#         for name, config in self.stream_configs.items():
#             if self.temporal_encoder == "bilstm":
...
#                 conv1d = TemporalConv(config['input_dim'], hidden_size, conv_type)
#                 contextual_module = TemporalTransformerEncoder(
#                 d_model=hidden_size,
#                 nhead=visual_args.get("tx_nhead", 8),
#                 num_layers=visual_args.get("tx_layers", 2),
#                 dim_feedforward=visual_args.get("tx_ffn_dim", hidden_size * 4),
#                 dropout=visual_args.get("tx_dropout", 0.1),
#                 use_sinusoidal_pos_emb=visual_args.get("tx_sinusoidal", True),
#             )
#             else:
...
#         if self.temporal_encoder == "bilstm":
#             contextual_feat = contextual_module(
#             conv1d_feat.transpose(0,1),
#             feat_len
#             )['predictions']
#         elif self.temporal_encoder == "transformer":
#             contextual_feat = contextual_module(
...
class KLdis(nn.Module):
    ...
    def forward(self, view1_logits, view2_logits, use_blank=True):
        ...
        view1_logits = F.log_softmax(
            view1_logits[:, :, start_idx:] / self.T, dim=-1
        ).reshape(-1, view2_logits.shape[2] - start_idx)

        ref_probs = F.softmax(
            view2_logits[:, :, start_idx:] / self.T, dim=-1
        ).reshape(-1, view2_logits.shape[2] - start_idx)
...
class TwoStream_Cosign(nn.Module):
    ...
    def forward_contextual(self, framewise, len_x, conv1d_module, contextual_module, classifier):
        ...
        visual_feat = conv1d_ret['visual_feat']
        feat_len = conv1d_ret['feat_len']

        B = len_x.shape[0]

        if visual_feat.shape[0] != B:
            visual_feat = visual_feat.transpose(0, 1)

        conv1d_feat = visual_feat  # [B,T,C]

        feat_len = feat_len.view(-1).long()


        if self.temporal_encoder == "bilstm":

            contextual_out = contextual_module(
                conv1d_feat.transpose(0,1),
                feat_len
            )

            if isinstance(contextual_out, dict):
                contextual_feat = contextual_out['predictions']
            else:
                contextual_feat = contextual_out
        ...
        conv1d_logits_bt = classifier(conv1d_feat)
        seq_logits_bt = classifier(contextual_feat)

        conv1d_logits = conv1d_logits_bt.permute(1, 0, 2)
        seq_logits = seq_logits_bt.permute(1, 0, 2)
    ...
    def forward(self, inputs_dict):
        ...
        if self.apply_CR and self.training:
            ...
            for stream_type in self.stream_configs.keys():
                ...
                view1 = visual_ret[f'view1_{stream_type}']
                view2 = visual_ret[f'view2_{stream_type}']
            ...
            conv1d_logits, seq_logits, feat_len = self.forward_contextual(
                fusion,
                len_x,
                self.conv1d_fusion,
            ...
            return {
                'conv_sents_fusion': decode_if_not_training(conv1d_logits),
                'recognized_sents_fusion': decode_if_not_training(seq_logits),
            }

    def get_ctc_loss(self, logits, label, feat_len, label_len):

        T, B, V = logits.shape

        feat_len = feat_len.view(-1)

        if feat_len.shape[0] != B:
            feat_len = feat_len[:B]

        return self.loss['ctc'](
            (logits * self.norm_scale).log_softmax(-1),
            label.cpu().int(),
            feat_len.cpu().int(),
            label_len.cpu().int(),
        ).mean()
    ...
    def get_loss(self, ret_dict, inputs_dict):
        ...
        label_lgt = inputs_dict['label_lgt']
        ...
        for k, weight in self.loss_weights.items():
            ...
            if loss_type in ['ConvCTC', 'SeqCTC']:
                ...
                view1_logits = ret_dict[f'view1_{stream_type}'][idx].permute(1, 0, 2) * self.norm_scale
                view2_logits = ret_dict[f'view2_{stream_type}'][idx].permute(1, 0, 2) * self.norm_scale
