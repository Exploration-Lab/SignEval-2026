...
class SkeletonFeeder(data.Dataset):
        ...
        if mode == 'test':
            ...
            for item in inputs_list:
                if item['video_id'] in self.kps_global.keys():
                    ...
                    print(item)
            missing = [it for it in inputs_list if it["video_id"] not in self.kps_global]
            print("missing:", len(missing))
            print("example missing ids:", [m["video_id"] for m in missing[:10]])
    ...
    def read_pose(self, index, num_glosses=-1):
        ...
        return pose_data, label_list, fi
